#!/usr/bin/env python3
"""
Simple test script to verify the device parameter fix without GUI dependencies.
"""
...
def test_render_settings_fix():
    """Test that render settings include device parameter."""
    print("🧪 Testing device parameter fix...")
    
    # Test the render settings that would be created by the main window
    render_settings = {
        'resolution_x': 1920,
        'resolution_y': 1080,
        'engine': 'CYCLES',
    ...
    if 'device' in render_settings:
        ...
        print(f"   Device: {render_settings['device']}")
    else:
        print("❌ Device parameter missing from render settings!")
        return False
    
    # Test commercial grade animator with these settings
    print("\n🧪 Testing CommercialGradeAnimator with device parameter...")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            animator.save_script(script_path, render_settings, blend_path)
            ...
            with open(script_path, 'r') as f:
                ...
                if "scene.cycles.device" in script_content:
                    ...
                    # Check if it uses the correct device value
                    if "scene.cycles.device = 'GPU'" in script_content:
                        print("✅ Generated script correctly sets device to GPU!")
                    else:
                        print("⚠️  Generated script device setting format may be incorrect")
                else:
    ...
    print("\n🎉 Device parameter fix test completed successfully!")
    return True

if __name__ == "__main__":
    success = test_render_settings_fix()
    if success:
