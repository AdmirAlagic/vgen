...
class AudioEngine: ObservableObject {
    ...
    deinit {
        // Cleanup is handled automatically when the object is deallocated
        // Manual cleanup should be called explicitly before deinit if needed
    }
    ...
    private func performAudioAnalysis() {
        guard let _ = audioBuffer, isPlaying else { return }
    ...
    private func performFFT(_ samples: [Float]) -> [Float] {
        ...
        return realInput.withUnsafeMutableBufferPointer { realPtr in
            return imaginaryInput.withUnsafeMutableBufferPointer { imagPtr in
                var complexInput = DSPSplitComplex(realp: realPtr.baseAddress!, imagp: imagPtr.baseAddress!)
                ...
                // Convert to dB manually
                let dbMagnitudes = magnitudes.map { magnitude in
                    let clampedMagnitude = max(magnitude, 1e-7) // Prevent log(0)
                    return 20.0 * log10(clampedMagnitude)
                }
