...
for name in shape_key_names:
    ...
    if "SimpleDeform" in name:
        # DRAMATIC MORPHING: Complex wave-like deformation instead of scaling
        for i, vert in enumerate(shape_key_data):
            # Create wave-like morphing pattern
            wave_x = math.sin(vert.co.x * 2.0) * 0.8
            wave_y = math.cos(vert.co.y * 2.0) * 0.8
            wave_z = math.sin(vert.co.z * 2.0) * 0.6
            
            # Apply dramatic morphing displacement
            vert.co.x += wave_x
            vert.co.y += wave_y
            vert.co.z += wave_z
            
    elif "Shrinkwrap" in name:
        # DRAMATIC MORPHING: Spiral-like deformation instead of shrinking
        for i, vert in enumerate(shape_key_data):
            # Create spiral morphing pattern
            angle = math.atan2(vert.co.y, vert.co.x)
            radius = math.sqrt(vert.co.x**2 + vert.co.y**2)
            
            # Apply spiral displacement
            spiral_factor = 1.2
            vert.co.x += math.cos(angle + radius * 0.5) * spiral_factor
            vert.co.y += math.sin(angle + radius * 0.5) * spiral_factor
            vert.co.z += math.sin(radius * 2.0) * 0.4
            
    elif "Wave" in name:
        # DRAMATIC MORPHING: Multi-layered wave deformation
        for i, vert in enumerate(shape_key_data):
            # Create complex multi-frequency wave pattern
            wave1 = math.sin(vert.co.x * 3.0) * 0.6
            wave2 = math.cos(vert.co.y * 3.0) * 0.6
            wave3 = math.sin(vert.co.z * 2.0) * 0.4
            wave4 = math.sin((vert.co.x + vert.co.y) * 1.5) * 0.3
            
            # Apply dramatic wave displacement
            vert.co.x += wave1 + wave4
            vert.co.y += wave2 + wave4
            vert.co.z += wave3
            
    elif "Displace" in name:
        # DRAMATIC MORPHING: Organic displacement patterns
        for i, vert in enumerate(shape_key_data):
            # Create organic displacement using noise-like patterns
            noise_x = math.sin(vert.co.x * 2.5 + vert.co.y * 1.5) * 0.8
            noise_y = math.cos(vert.co.y * 2.5 + vert.co.z * 1.5) * 0.8
            noise_z = math.sin(vert.co.z * 2.5 + vert.co.x * 1.5) * 0.6
            
            # Add secondary displacement for complexity
            secondary_x = math.sin(vert.co.x * 4.0) * 0.3
            secondary_y = math.cos(vert.co.y * 4.0) * 0.3
            secondary_z = math.sin(vert.co.z * 4.0) * 0.2
            
            # Apply dramatic organic displacement
            vert.co.x += noise_x + secondary_x
            vert.co.y += noise_y + secondary_y
            vert.co.z += noise_z + secondary_z
...
# Animate SimpleDeform using SMOOTH keyframe insertion (no frame jumping)
shape_key = cube.data.shape_keys.key_blocks["SimpleDeform"]

# Set shape key value and insert keyframe for frame 0 (no frame jumping)
shape_key.value = -0.40854065293843433
shape_key.keyframe_insert(data_path="value", frame=0)

# Set shape key value and insert keyframe for frame 6 (no frame jumping)
shape_key.value = -2.1382175531401857
shape_key.keyframe_insert(data_path="value", frame=6)

# Set shape key value and insert keyframe for frame 12 (no frame jumping)
shape_key.value = -2.16804129715532
shape_key.keyframe_insert(data_path="value", frame=12)

# Set shape key value and insert keyframe for frame 18 (no frame jumping)
shape_key.value = -1.1030691036458715
shape_key.keyframe_insert(data_path="value", frame=18)

# Set shape key value and insert keyframe for frame 24 (no frame jumping)
shape_key.value = -0.4008952471126652
shape_key.keyframe_insert(data_path="value", frame=24)

# Set shape key value and insert keyframe for frame 30 (no frame jumping)
shape_key.value = -1.4355168199573671
shape_key.keyframe_insert(data_path="value", frame=30)

# Set shape key value and insert keyframe for frame 36 (no frame jumping)
shape_key.value = -2.484224697244862
shape_key.keyframe_insert(data_path="value", frame=36)

# Set shape key value and insert keyframe for frame 42 (no frame jumping)
shape_key.value = -2.0414220125913034
shape_key.keyframe_insert(data_path="value", frame=42)

# Set shape key value and insert keyframe for frame 48 (no frame jumping)
shape_key.value = -0.5137447207104606
shape_key.keyframe_insert(data_path="value", frame=48)

# Set shape key value and insert keyframe for frame 54 (no frame jumping)
shape_key.value = 1.5586109532646528
shape_key.keyframe_insert(data_path="value", frame=54)

# Set shape key value and insert keyframe for frame 60 (no frame jumping)
shape_key.value = 2.5
shape_key.keyframe_insert(data_path="value", frame=60)

# Set shape key value and insert keyframe for frame 66 (no frame jumping)
shape_key.value = 0.47947425369302277
shape_key.keyframe_insert(data_path="value", frame=66)

# Set shape key value and insert keyframe for frame 72 (no frame jumping)
shape_key.value = -1.6195305649701948
shape_key.keyframe_insert(data_path="value", frame=72)

# Set shape key value and insert keyframe for frame 78 (no frame jumping)
shape_key.value = -1.7830542408937333
shape_key.keyframe_insert(data_path="value", frame=78)

# Set shape key value and insert keyframe for frame 84 (no frame jumping)
shape_key.value = -1.0666276207882321
shape_key.keyframe_insert(data_path="value", frame=84)

# Set shape key value and insert keyframe for frame 90 (no frame jumping)
shape_key.value = -0.3492957069511071
shape_key.keyframe_insert(data_path="value", frame=90)

# Set shape key value and insert keyframe for frame 96 (no frame jumping)
shape_key.value = -0.3694316010289893
shape_key.keyframe_insert(data_path="value", frame=96)

# Set shape key value and insert keyframe for frame 102 (no frame jumping)
shape_key.value = -1.5907049456833653
shape_key.keyframe_insert(data_path="value", frame=102)

# Set shape key value and insert keyframe for frame 108 (no frame jumping)
shape_key.value = -2.5
shape_key.keyframe_insert(data_path="value", frame=108)

# Set shape key value and insert keyframe for frame 114 (no frame jumping)
shape_key.value = -2.295483665535168
shape_key.keyframe_insert(data_path="value", frame=114)

# Set shape key value and insert keyframe for frame 120 (no frame jumping)
shape_key.value = -1.3037150080387039
shape_key.keyframe_insert(data_path="value", frame=120)

# Set shape key value and insert keyframe for frame 126 (no frame jumping)
shape_key.value = -0.40365719671283395
shape_key.keyframe_insert(data_path="value", frame=126)

# Set shape key value and insert keyframe for frame 132 (no frame jumping)
shape_key.value = -0.0955331365214871
shape_key.keyframe_insert(data_path="value", frame=132)

# Set shape key value and insert keyframe for frame 138 (no frame jumping)
shape_key.value = -0.8743306174987986
shape_key.keyframe_insert(data_path="value", frame=138)

# Set shape key value and insert keyframe for frame 144 (no frame jumping)
shape_key.value = -2.0523567165657575
shape_key.keyframe_insert(data_path="value", frame=144)

# Set shape key value and insert keyframe for frame 150 (no frame jumping)
shape_key.value = -2.3401255219092025
shape_key.keyframe_insert(data_path="value", frame=150)

# Set shape key value and insert keyframe for frame 156 (no frame jumping)
shape_key.value = -1.6180246649955716
shape_key.keyframe_insert(data_path="value", frame=156)

# Set shape key value and insert keyframe for frame 162 (no frame jumping)
shape_key.value = -0.2655055250682632
shape_key.keyframe_insert(data_path="value", frame=162)

# Set shape key value and insert keyframe for frame 168 (no frame jumping)
shape_key.value = 0.6599155179411955
shape_key.keyframe_insert(data_path="value", frame=168)

# Set shape key value and insert keyframe for frame 174 (no frame jumping)
shape_key.value = -0.11902179360721865
shape_key.keyframe_insert(data_path="value", frame=174)

# Set shape key value and insert keyframe for frame 180 (no frame jumping)
shape_key.value = -1.7683342708906338
shape_key.keyframe_insert(data_path="value", frame=180)

# Set shape key value and insert keyframe for frame 186 (no frame jumping)
shape_key.value = -2.2066211528876156
shape_key.keyframe_insert(data_path="value", frame=186)

# Set shape key value and insert keyframe for frame 192 (no frame jumping)
shape_key.value = -1.4302072151800407
shape_key.keyframe_insert(data_path="value", frame=192)

# Set shape key value and insert keyframe for frame 198 (no frame jumping)
shape_key.value = 0.11495210959776836
shape_key.keyframe_insert(data_path="value", frame=198)

# Set shape key value and insert keyframe for frame 204 (no frame jumping)
shape_key.value = 1.8094523951148815
shape_key.keyframe_insert(data_path="value", frame=204)

# Set shape key value and insert keyframe for frame 210 (no frame jumping)
shape_key.value = 2.030252652373478
shape_key.keyframe_insert(data_path="value", frame=210)

# Set shape key value and insert keyframe for frame 216 (no frame jumping)
shape_key.value = 0.02061019827339816
shape_key.keyframe_insert(data_path="value", frame=216)

# Set shape key value and insert keyframe for frame 222 (no frame jumping)
shape_key.value = -1.050012926620668
shape_key.keyframe_insert(data_path="value", frame=222)

# Set shape key value and insert keyframe for frame 228 (no frame jumping)
shape_key.value = -0.813340998395532
shape_key.keyframe_insert(data_path="value", frame=228)

# Set shape key value and insert keyframe for frame 234 (no frame jumping)
shape_key.value = -0.1921426731858218
shape_key.keyframe_insert(data_path="value", frame=234)


# Animate SimpleDeform.001 using SMOOTH keyframe insertion (no frame jumping)
...
# Set shape key value and insert keyframe for frame 6 (no frame jumping)
shape_key.value = -0.15074770708883567
shape_key.keyframe_insert(data_path="value", frame=6)

# Set shape key value and insert keyframe for frame 12 (no frame jumping)
shape_key.value = -0.7821552390627446
shape_key.keyframe_insert(data_path="value", frame=12)

# Set shape key value and insert keyframe for frame 18 (no frame jumping)
shape_key.value = -0.6943005889433742
shape_key.keyframe_insert(data_path="value", frame=18)

# Set shape key value and insert keyframe for frame 24 (no frame jumping)
shape_key.value = -0.7896493307821308
shape_key.keyframe_insert(data_path="value", frame=24)

# Set shape key value and insert keyframe for frame 30 (no frame jumping)
shape_key.value = -1.2793978978176077
shape_key.keyframe_insert(data_path="value", frame=30)

# Set shape key value and insert keyframe for frame 36 (no frame jumping)
shape_key.value = -1.5756675974862464
shape_key.keyframe_insert(data_path="value", frame=36)

# Set shape key value and insert keyframe for frame 42 (no frame jumping)
shape_key.value = -1.3992846093524867
shape_key.keyframe_insert(data_path="value", frame=42)

# Set shape key value and insert keyframe for frame 48 (no frame jumping)
shape_key.value = -1.135535299413804
shape_key.keyframe_insert(data_path="value", frame=48)

# Set shape key value and insert keyframe for frame 54 (no frame jumping)
shape_key.value = -1.0381240276359402
shape_key.keyframe_insert(data_path="value", frame=54)

# Set shape key value and insert keyframe for frame 60 (no frame jumping)
shape_key.value = -1.282082331494617
shape_key.keyframe_insert(data_path="value", frame=60)

# Set shape key value and insert keyframe for frame 66 (no frame jumping)
shape_key.value = -1.715191203306798
shape_key.keyframe_insert(data_path="value", frame=66)

# Set shape key value and insert keyframe for frame 72 (no frame jumping)
shape_key.value = -1.952178789277532
shape_key.keyframe_insert(data_path="value", frame=72)

# Set shape key value and insert keyframe for frame 78 (no frame jumping)
shape_key.value = -1.9649646224363797
shape_key.keyframe_insert(data_path="value", frame=78)

# Set shape key value and insert keyframe for frame 84 (no frame jumping)
shape_key.value = -1.8865832047716753
shape_key.keyframe_insert(data_path="value", frame=84)

# Set shape key value and insert keyframe for frame 90 (no frame jumping)
shape_key.value = -1.8326748752736055
shape_key.keyframe_insert(data_path="value", frame=90)

# Set shape key value and insert keyframe for frame 96 (no frame jumping)
shape_key.value = -1.911438021499717
shape_key.keyframe_insert(data_path="value", frame=96)
...
# Set shape key value and insert keyframe for frame 108 (no frame jumping)
shape_key.value = -1.9853325033721734
shape_key.keyframe_insert(data_path="value", frame=108)

# Set shape key value and insert keyframe for frame 114 (no frame jumping)
shape_key.value = -1.7557671977548965
shape_key.keyframe_insert(data_path="value", frame=114)

# Set shape key value and insert keyframe for frame 120 (no frame jumping)
shape_key.value = -1.4256536708854832
shape_key.keyframe_insert(data_path="value", frame=120)

# Set shape key value and insert keyframe for frame 126 (no frame jumping)
shape_key.value = -1.2356405312238072
shape_key.keyframe_insert(data_path="value", frame=126)

# Set shape key value and insert keyframe for frame 132 (no frame jumping)
shape_key.value = -1.3122395568572682
shape_key.keyframe_insert(data_path="value", frame=132)

# Set shape key value and insert keyframe for frame 138 (no frame jumping)
shape_key.value = -1.5573157627141558
shape_key.keyframe_insert(data_path="value", frame=138)

# Set shape key value and insert keyframe for frame 144 (no frame jumping)
shape_key.value = -1.7731640476888992
shape_key.keyframe_insert(data_path="value", frame=144)

# Set shape key value and insert keyframe for frame 150 (no frame jumping)
shape_key.value = -1.762380958037775
shape_key.keyframe_insert(data_path="value", frame=150)

# Set shape key value and insert keyframe for frame 156 (no frame jumping)
shape_key.value = -1.6428281871050094
shape_key.keyframe_insert(data_path="value", frame=156)

# Set shape key value and insert keyframe for frame 162 (no frame jumping)
shape_key.value = -1.4626844864752255
shape_key.keyframe_insert(data_path="value", frame=162)

# Set shape key value and insert keyframe for frame 168 (no frame jumping)
shape_key.value = -0.9430516077880944
shape_key.keyframe_insert(data_path="value", frame=168)

# Set shape key value and insert keyframe for frame 174 (no frame jumping)
shape_key.value = -0.262748270157692
shape_key.keyframe_insert(data_path="value", frame=174)

# Set shape key value and insert keyframe for frame 180 (no frame jumping)
shape_key.value = -0.08071961745698197
shape_key.keyframe_insert(data_path="value", frame=180)

# Set shape key value and insert keyframe for frame 186 (no frame jumping)
shape_key.value = -0.3380173233494781
shape_key.keyframe_insert(data_path="value", frame=186)

# Set shape key value and insert keyframe for frame 192 (no frame jumping)
shape_key.value = -0.36803494430389394
shape_key.keyframe_insert(data_path="value", frame=192)

# Set shape key value and insert keyframe for frame 198 (no frame jumping)
shape_key.value = -0.19077321493475607
shape_key.keyframe_insert(data_path="value", frame=198)

# Set shape key value and insert keyframe for frame 204 (no frame jumping)
shape_key.value = 0.24672613065853888
shape_key.keyframe_insert(data_path="value", frame=204)

# Set shape key value and insert keyframe for frame 210 (no frame jumping)
shape_key.value = 0.7738455608255155
shape_key.keyframe_insert(data_path="value", frame=210)

# Set shape key value and insert keyframe for frame 216 (no frame jumping)
shape_key.value = 0.7819828637699016
shape_key.keyframe_insert(data_path="value", frame=216)

# Set shape key value and insert keyframe for frame 222 (no frame jumping)
shape_key.value = 0.32339223479446444
shape_key.keyframe_insert(data_path="value", frame=222)

# Set shape key value and insert keyframe for frame 228 (no frame jumping)
shape_key.value = 0.06471145104681852
shape_key.keyframe_insert(data_path="value", frame=228)

# Set shape key value and insert keyframe for frame 234 (no frame jumping)
shape_key.value = -0.16699983672530538
shape_key.keyframe_insert(data_path="value", frame=234)


# Animate Shrinkwrap using SMOOTH keyframe insertion (no frame jumping)
...
# Set shape key value and insert keyframe for frame 6 (no frame jumping)
shape_key.value = -1.758998639281348
shape_key.keyframe_insert(data_path="value", frame=6)
...
# Set shape key value and insert keyframe for frame 126 (no frame jumping)
shape_key.value = -2.121530112058965
shape_key.keyframe_insert(data_path="value", frame=126)

# Set shape key value and insert keyframe for frame 132 (no frame jumping)
shape_key.value = -2.0043775299275466
shape_key.keyframe_insert(data_path="value", frame=132)

# Set shape key value and insert keyframe for frame 138 (no frame jumping)
shape_key.value = -1.9367051078162618
shape_key.keyframe_insert(data_path="value", frame=138)

# Set shape key value and insert keyframe for frame 144 (no frame jumping)
shape_key.value = -1.923119236990289
shape_key.keyframe_insert(data_path="value", frame=144)

# Set shape key value and insert keyframe for frame 150 (no frame jumping)
shape_key.value = -1.9577461306681374
shape_key.keyframe_insert(data_path="value", frame=150)

# Set shape key value and insert keyframe for frame 156 (no frame jumping)
shape_key.value = -2.025879979285697
shape_key.keyframe_insert(data_path="value", frame=156)

# Set shape key value and insert keyframe for frame 162 (no frame jumping)
shape_key.value = -2.107389735907016
shape_key.keyframe_insert(data_path="value", frame=162)

# Set shape key value and insert keyframe for frame 168 (no frame jumping)
shape_key.value = -2.1811463794403645
shape_key.keyframe_insert(data_path="value", frame=168)
...
# Set shape key value and insert keyframe for frame 180 (no frame jumping)
shape_key.value = -1.5990995648310407
shape_key.keyframe_insert(data_path="value", frame=180)

# Set shape key value and insert keyframe for frame 186 (no frame jumping)
shape_key.value = -1.7809849061861673
shape_key.keyframe_insert(data_path="value", frame=186)

# Set shape key value and insert keyframe for frame 192 (no frame jumping)
shape_key.value = -2.1667064352903616
shape_key.keyframe_insert(data_path="value", frame=192)

# Set shape key value and insert keyframe for frame 198 (no frame jumping)
shape_key.value = -2.103138102877733
shape_key.keyframe_insert(data_path="value", frame=198)

# Set shape key value and insert keyframe for frame 204 (no frame jumping)
shape_key.value = -1.9736558729204723
shape_key.keyframe_insert(data_path="value", frame=204)

# Set shape key value and insert keyframe for frame 210 (no frame jumping)
shape_key.value = -1.1649335295629912
shape_key.keyframe_insert(data_path="value", frame=210)

# Set shape key value and insert keyframe for frame 216 (no frame jumping)
shape_key.value = -0.6387806407414554
shape_key.keyframe_insert(data_path="value", frame=216)

# Set shape key value and insert keyframe for frame 222 (no frame jumping)
shape_key.value = -0.9660707875229593
shape_key.keyframe_insert(data_path="value", frame=222)

# Set shape key value and insert keyframe for frame 228 (no frame jumping)
shape_key.value = -1.6735954543953147
shape_key.keyframe_insert(data_path="value", frame=228)

# Set shape key value and insert keyframe for frame 234 (no frame jumping)
shape_key.value = -2.0756601183176593
shape_key.keyframe_insert(data_path="value", frame=234)


# Animate Shrinkwrap.001 using SMOOTH keyframe insertion (no frame jumping)
shape_key = cube.data.shape_keys.key_blocks["Shrinkwrap.001"]

# Set shape key value and insert keyframe for frame 0 (no frame jumping)
shape_key.value = -0.47579958670784217
shape_key.keyframe_insert(data_path="value", frame=0)

# Set shape key value and insert keyframe for frame 6 (no frame jumping)
shape_key.value = -1.6129530454927674
shape_key.keyframe_insert(data_path="value", frame=6)

# Set shape key value and insert keyframe for frame 12 (no frame jumping)
shape_key.value = -1.5995537424220574
shape_key.keyframe_insert(data_path="value", frame=12)

# Set shape key value and insert keyframe for frame 18 (no frame jumping)
shape_key.value = -0.6836939933614392
shape_key.keyframe_insert(data_path="value", frame=18)

# Set shape key value and insert keyframe for frame 24 (no frame jumping)
shape_key.value = 0.07617805167641589
shape_key.keyframe_insert(data_path="value", frame=24)

# Set shape key value and insert keyframe for frame 30 (no frame jumping)
shape_key.value = -0.505591702287844
shape_key.keyframe_insert(data_path="value", frame=30)

# Set shape key value and insert keyframe for frame 36 (no frame jumping)
shape_key.value = -1.4555420058882884
shape_key.keyframe_insert(data_path="value", frame=36)

# Set shape key value and insert keyframe for frame 42 (no frame jumping)
shape_key.value = -1.2140419112267393
shape_key.keyframe_insert(data_path="value", frame=42)

# Set shape key value and insert keyframe for frame 48 (no frame jumping)
shape_key.value = -0.10924777256174223
shape_key.keyframe_insert(data_path="value", frame=48)

# Set shape key value and insert keyframe for frame 54 (no frame jumping)
shape_key.value = 1.215473486836715
shape_key.keyframe_insert(data_path="value", frame=54)
...
# Set shape key value and insert keyframe for frame 66 (no frame jumping)
shape_key.value = 0.77564072898362
shape_key.keyframe_insert(data_path="value", frame=66)

# Set shape key value and insert keyframe for frame 72 (no frame jumping)
shape_key.value = -0.825155938161559
shape_key.keyframe_insert(data_path="value", frame=72)

# Set shape key value and insert keyframe for frame 78 (no frame jumping)
shape_key.value = -1.2144062174088726
shape_key.keyframe_insert(data_path="value", frame=78)

# Set shape key value and insert keyframe for frame 84 (no frame jumping)
shape_key.value = -0.7992826884309598
shape_key.keyframe_insert(data_path="value", frame=84)

# Set shape key value and insert keyframe for frame 90 (no frame jumping)
shape_key.value = -0.205049030143454
shape_key.keyframe_insert(data_path="value", frame=90)

# Set shape key value and insert keyframe for frame 96 (no frame jumping)
shape_key.value = -0.021403504219993597
shape_key.keyframe_insert(data_path="value", frame=96)

# Set shape key value and insert keyframe for frame 102 (no frame jumping)
shape_key.value = -0.8981491459988988
shape_key.keyframe_insert(data_path="value", frame=102)
...
# Set shape key value and insert keyframe for frame 114 (no frame jumping)
shape_key.value = -1.7867657409730082
shape_key.keyframe_insert(data_path="value", frame=114)

# Set shape key value and insert keyframe for frame 120 (no frame jumping)
shape_key.value = -1.1602471058090407
shape_key.keyframe_insert(data_path="value", frame=120)

# Set shape key value and insert keyframe for frame 126 (no frame jumping)
shape_key.value = -0.4319253252113433
shape_key.keyframe_insert(data_path="value", frame=126)

# Set shape key value and insert keyframe for frame 132 (no frame jumping)
shape_key.value = -0.07088677924579079
shape_key.keyframe_insert(data_path="value", frame=132)

# Set shape key value and insert keyframe for frame 138 (no frame jumping)
shape_key.value = -0.5366498944755629
shape_key.keyframe_insert(data_path="value", frame=138)

# Set shape key value and insert keyframe for frame 144 (no frame jumping)
shape_key.value = -1.373794450903451
shape_key.keyframe_insert(data_path="value", frame=144)

# Set shape key value and insert keyframe for frame 150 (no frame jumping)
shape_key.value = -1.5327370443025654
shape_key.keyframe_insert(data_path="value", frame=150)

# Set shape key value and insert keyframe for frame 156 (no frame jumping)
shape_key.value = -0.9298742001771945
shape_key.keyframe_insert(data_path="value", frame=156)

# Set shape key value and insert keyframe for frame 162 (no frame jumping)
shape_key.value = 0.1902369722093511
shape_key.keyframe_insert(data_path="value", frame=162)

# Set shape key value and insert keyframe for frame 168 (no frame jumping)
shape_key.value = 1.0670909945667109
shape_key.keyframe_insert(data_path="value", frame=168)

# Set shape key value and insert keyframe for frame 174 (no frame jumping)
shape_key.value = 0.6692006553585783
shape_key.keyframe_insert(data_path="value", frame=174)

# Set shape key value and insert keyframe for frame 180 (no frame jumping)
shape_key.value = -0.7791999640034117
shape_key.keyframe_insert(data_path="value", frame=180)

# Set shape key value and insert keyframe for frame 186 (no frame jumping)
shape_key.value = -1.3536053818191154
shape_key.keyframe_insert(data_path="value", frame=186)

# Set shape key value and insert keyframe for frame 192 (no frame jumping)
shape_key.value = -0.8444746292344559
shape_key.keyframe_insert(data_path="value", frame=192)

# Set shape key value and insert keyframe for frame 198 (no frame jumping)
shape_key.value = 0.26392407985231015
shape_key.keyframe_insert(data_path="value", frame=198)

# Set shape key value and insert keyframe for frame 204 (no frame jumping)
shape_key.value = 1.31692545087578
shape_key.keyframe_insert(data_path="value", frame=204)

# Set shape key value and insert keyframe for frame 210 (no frame jumping)
shape_key.value = 1.5476612454813508
shape_key.keyframe_insert(data_path="value", frame=210)

# Set shape key value and insert keyframe for frame 216 (no frame jumping)
shape_key.value = 0.34970830757968296
shape_key.keyframe_insert(data_path="value", frame=216)

# Set shape key value and insert keyframe for frame 222 (no frame jumping)
shape_key.value = -0.6606973252336925
shape_key.keyframe_insert(data_path="value", frame=222)

# Set shape key value and insert keyframe for frame 228 (no frame jumping)
shape_key.value = -0.6552497915558146
shape_key.keyframe_insert(data_path="value", frame=228)

# Set shape key value and insert keyframe for frame 234 (no frame jumping)
shape_key.value = -0.24968678360661145
shape_key.keyframe_insert(data_path="value", frame=234)


# Animate Shrinkwrap.002 using SMOOTH keyframe insertion (no frame jumping)
...
# Set shape key value and insert keyframe for frame 6 (no frame jumping)
shape_key.value = -0.4841610444446942
shape_key.keyframe_insert(data_path="value", frame=6)

# Set shape key value and insert keyframe for frame 12 (no frame jumping)
shape_key.value = -1.4874451828412572
shape_key.keyframe_insert(data_path="value", frame=12)
...
# Set shape key value and insert keyframe for frame 114 (no frame jumping)
shape_key.value = -1.4995171035025439
shape_key.keyframe_insert(data_path="value", frame=114)

# Set shape key value and insert keyframe for frame 120 (no frame jumping)
shape_key.value = -1.3811492266964303
shape_key.keyframe_insert(data_path="value", frame=120)

# Set shape key value and insert keyframe for frame 126 (no frame jumping)
shape_key.value = -1.3362679810743288
shape_key.keyframe_insert(data_path="value", frame=126)

# Set shape key value and insert keyframe for frame 132 (no frame jumping)
shape_key.value = -1.2984576032716895
shape_key.keyframe_insert(data_path="value", frame=132)

# Set shape key value and insert keyframe for frame 138 (no frame jumping)
shape_key.value = -1.2661342165423723
shape_key.keyframe_insert(data_path="value", frame=138)

# Set shape key value and insert keyframe for frame 144 (no frame jumping)
shape_key.value = -1.2711695091101003
shape_key.keyframe_insert(data_path="value", frame=144)

# Set shape key value and insert keyframe for frame 150 (no frame jumping)
shape_key.value = -1.3129989946029679
shape_key.keyframe_insert(data_path="value", frame=150)

# Set shape key value and insert keyframe for frame 156 (no frame jumping)
shape_key.value = -1.3760269404235128
shape_key.keyframe_insert(data_path="value", frame=156)

# Set shape key value and insert keyframe for frame 162 (no frame jumping)
shape_key.value = -1.4337161597757004
shape_key.keyframe_insert(data_path="value", frame=162)

# Set shape key value and insert keyframe for frame 168 (no frame jumping)
shape_key.value = -1.4166132987521551
shape_key.keyframe_insert(data_path="value", frame=168)

# Set shape key value and insert keyframe for frame 174 (no frame jumping)
shape_key.value = -1.2933874741657203
shape_key.keyframe_insert(data_path="value", frame=174)

# Set shape key value and insert keyframe for frame 180 (no frame jumping)
shape_key.value = -1.1504572388897092
shape_key.keyframe_insert(data_path="value", frame=180)

# Set shape key value and insert keyframe for frame 186 (no frame jumping)
shape_key.value = -1.0364214157114602
shape_key.keyframe_insert(data_path="value", frame=186)

# Set shape key value and insert keyframe for frame 192 (no frame jumping)
shape_key.value = -0.9761642376763147
shape_key.keyframe_insert(data_path="value", frame=192)

# Set shape key value and insert keyframe for frame 198 (no frame jumping)
shape_key.value = -0.90034613768945
shape_key.keyframe_insert(data_path="value", frame=198)

# Set shape key value and insert keyframe for frame 204 (no frame jumping)
shape_key.value = -0.7435349445423831
shape_key.keyframe_insert(data_path="value", frame=204)

# Set shape key value and insert keyframe for frame 210 (no frame jumping)
shape_key.value = -0.5518477968966458
shape_key.keyframe_insert(data_path="value", frame=210)

# Set shape key value and insert keyframe for frame 216 (no frame jumping)
shape_key.value = -0.44087988531131894
shape_key.keyframe_insert(data_path="value", frame=216)

# Set shape key value and insert keyframe for frame 222 (no frame jumping)
shape_key.value = -0.463841410940681
shape_key.keyframe_insert(data_path="value", frame=222)

# Set shape key value and insert keyframe for frame 228 (no frame jumping)
shape_key.value = -0.606941632494292
shape_key.keyframe_insert(data_path="value", frame=228)

# Set shape key value and insert keyframe for frame 234 (no frame jumping)
shape_key.value = -0.7491817348266907
shape_key.keyframe_insert(data_path="value", frame=234)


# Animate Wave using SMOOTH keyframe insertion (no frame jumping)
...
# Set shape key value and insert keyframe for frame 6 (no frame jumping)
shape_key.value = -0.47547398488666626
shape_key.keyframe_insert(data_path="value", frame=6)

# Set shape key value and insert keyframe for frame 12 (no frame jumping)
shape_key.value = -1.776603241295298
shape_key.keyframe_insert(data_path="value", frame=12)

# Set shape key value and insert keyframe for frame 18 (no frame jumping)
shape_key.value = -1.9410118844431974
shape_key.keyframe_insert(data_path="value", frame=18)
...
# Set shape key value and insert keyframe for frame 54 (no frame jumping)
shape_key.value = -1.9462262698028705
shape_key.keyframe_insert(data_path="value", frame=54)

# Set shape key value and insert keyframe for frame 60 (no frame jumping)
shape_key.value = -1.995521719931817
shape_key.keyframe_insert(data_path="value", frame=60)
...
# Set shape key value and insert keyframe for frame 114 (no frame jumping)
shape_key.value = -1.93624388621628
shape_key.keyframe_insert(data_path="value", frame=114)

# Set shape key value and insert keyframe for frame 120 (no frame jumping)
shape_key.value = -1.748206920860413
shape_key.keyframe_insert(data_path="value", frame=120)

# Set shape key value and insert keyframe for frame 126 (no frame jumping)
shape_key.value = -1.6489788594373027
shape_key.keyframe_insert(data_path="value", frame=126)

# Set shape key value and insert keyframe for frame 132 (no frame jumping)
shape_key.value = -1.6545255942977444
shape_key.keyframe_insert(data_path="value", frame=132)

# Set shape key value and insert keyframe for frame 138 (no frame jumping)
shape_key.value = -1.723970412660016
shape_key.keyframe_insert(data_path="value", frame=138)

# Set shape key value and insert keyframe for frame 144 (no frame jumping)
shape_key.value = -1.7729152227450797
shape_key.keyframe_insert(data_path="value", frame=144)

# Set shape key value and insert keyframe for frame 150 (no frame jumping)
shape_key.value = -1.7764189089089826
shape_key.keyframe_insert(data_path="value", frame=150)

# Set shape key value and insert keyframe for frame 156 (no frame jumping)
shape_key.value = -1.8013300231493505
shape_key.keyframe_insert(data_path="value", frame=156)

# Set shape key value and insert keyframe for frame 162 (no frame jumping)
shape_key.value = -1.7758562600147048
shape_key.keyframe_insert(data_path="value", frame=162)

# Set shape key value and insert keyframe for frame 168 (no frame jumping)
shape_key.value = -1.4138931689030798
shape_key.keyframe_insert(data_path="value", frame=168)

# Set shape key value and insert keyframe for frame 174 (no frame jumping)
shape_key.value = -0.8003571828920566
shape_key.keyframe_insert(data_path="value", frame=174)

# Set shape key value and insert keyframe for frame 180 (no frame jumping)
shape_key.value = -0.3834800908673597
shape_key.keyframe_insert(data_path="value", frame=180)

# Set shape key value and insert keyframe for frame 186 (no frame jumping)
shape_key.value = -0.5297408192342052
shape_key.keyframe_insert(data_path="value", frame=186)

# Set shape key value and insert keyframe for frame 192 (no frame jumping)
shape_key.value = -0.9142344897652186
shape_key.keyframe_insert(data_path="value", frame=192)

# Set shape key value and insert keyframe for frame 198 (no frame jumping)
shape_key.value = -0.9272590126674778
shape_key.keyframe_insert(data_path="value", frame=198)

# Set shape key value and insert keyframe for frame 204 (no frame jumping)
shape_key.value = -0.5761540853819631
shape_key.keyframe_insert(data_path="value", frame=204)

# Set shape key value and insert keyframe for frame 210 (no frame jumping)
shape_key.value = -0.03389182334381413
shape_key.keyframe_insert(data_path="value", frame=210)

# Set shape key value and insert keyframe for frame 216 (no frame jumping)
shape_key.value = 0.29535092991532563
shape_key.keyframe_insert(data_path="value", frame=216)

# Set shape key value and insert keyframe for frame 222 (no frame jumping)
shape_key.value = 0.08014928582305547
shape_key.keyframe_insert(data_path="value", frame=222)

# Set shape key value and insert keyframe for frame 228 (no frame jumping)
shape_key.value = -0.3869987482594972
shape_key.keyframe_insert(data_path="value", frame=228)

# Set shape key value and insert keyframe for frame 234 (no frame jumping)
shape_key.value = -0.676321025118116
shape_key.keyframe_insert(data_path="value", frame=234)


# Animate Displace using SMOOTH keyframe insertion (no frame jumping)
shape_key = cube.data.shape_keys.key_blocks["Displace"]

# Set shape key value and insert keyframe for frame 0 (no frame jumping)
shape_key.value = -0.8832797311512961
shape_key.keyframe_insert(data_path="value", frame=0)

# Set shape key value and insert keyframe for frame 6 (no frame jumping)
shape_key.value = -2.322314968743862
shape_key.keyframe_insert(data_path="value", frame=6)

# Set shape key value and insert keyframe for frame 12 (no frame jumping)
shape_key.value = -2.217709627837927
shape_key.keyframe_insert(data_path="value", frame=12)

# Set shape key value and insert keyframe for frame 18 (no frame jumping)
shape_key.value = -0.8971101864672355
shape_key.keyframe_insert(data_path="value", frame=18)

# Set shape key value and insert keyframe for frame 24 (no frame jumping)
shape_key.value = 0.180184964574444
shape_key.keyframe_insert(data_path="value", frame=24)

# Set shape key value and insert keyframe for frame 30 (no frame jumping)
shape_key.value = -0.6291589649989033
shape_key.keyframe_insert(data_path="value", frame=30)

# Set shape key value and insert keyframe for frame 36 (no frame jumping)
shape_key.value = -1.9646516972826968
shape_key.keyframe_insert(data_path="value", frame=36)

# Set shape key value and insert keyframe for frame 42 (no frame jumping)
shape_key.value = -1.6313692948366842
shape_key.keyframe_insert(data_path="value", frame=42)

# Set shape key value and insert keyframe for frame 48 (no frame jumping)
shape_key.value = -0.09941611781996551
shape_key.keyframe_insert(data_path="value", frame=48)

# Set shape key value and insert keyframe for frame 54 (no frame jumping)
shape_key.value = 1.7182481437194292
shape_key.keyframe_insert(data_path="value", frame=54)
...
# Set shape key value and insert keyframe for frame 66 (no frame jumping)
shape_key.value = 1.1150476551470048
shape_key.keyframe_insert(data_path="value", frame=66)

# Set shape key value and insert keyframe for frame 72 (no frame jumping)
shape_key.value = -1.0949290752665564
shape_key.keyframe_insert(data_path="value", frame=72)

# Set shape key value and insert keyframe for frame 78 (no frame jumping)
shape_key.value = -1.627044841179388
shape_key.keyframe_insert(data_path="value", frame=78)

# Set shape key value and insert keyframe for frame 84 (no frame jumping)
shape_key.value = -1.0394402070470714
shape_key.keyframe_insert(data_path="value", frame=84)

# Set shape key value and insert keyframe for frame 90 (no frame jumping)
shape_key.value = -0.21079797595697292
shape_key.keyframe_insert(data_path="value", frame=90)

# Set shape key value and insert keyframe for frame 96 (no frame jumping)
shape_key.value = 0.03268981936182236
shape_key.keyframe_insert(data_path="value", frame=96)

# Set shape key value and insert keyframe for frame 102 (no frame jumping)
shape_key.value = -1.193797384824329
shape_key.keyframe_insert(data_path="value", frame=102)

# Set shape key value and insert keyframe for frame 108 (no frame jumping)
shape_key.value = -2.499688774831694
shape_key.keyframe_insert(data_path="value", frame=108)

# Set shape key value and insert keyframe for frame 114 (no frame jumping)
shape_key.value = -2.4685495350094544
shape_key.keyframe_insert(data_path="value", frame=114)

# Set shape key value and insert keyframe for frame 120 (no frame jumping)
shape_key.value = -1.5896565467861683
shape_key.keyframe_insert(data_path="value", frame=120)

# Set shape key value and insert keyframe for frame 126 (no frame jumping)
shape_key.value = -0.574756057606949
shape_key.keyframe_insert(data_path="value", frame=126)

# Set shape key value and insert keyframe for frame 132 (no frame jumping)
shape_key.value = -0.07781758067295685
shape_key.keyframe_insert(data_path="value", frame=132)

# Set shape key value and insert keyframe for frame 138 (no frame jumping)
shape_key.value = -0.7305581671785097
shape_key.keyframe_insert(data_path="value", frame=138)

# Set shape key value and insert keyframe for frame 144 (no frame jumping)
shape_key.value = -1.9042356026132399
shape_key.keyframe_insert(data_path="value", frame=144)

# Set shape key value and insert keyframe for frame 150 (no frame jumping)
shape_key.value = -2.1245230636815835
shape_key.keyframe_insert(data_path="value", frame=150)

# Set shape key value and insert keyframe for frame 156 (no frame jumping)
shape_key.value = -1.2780941648701667
shape_key.keyframe_insert(data_path="value", frame=156)

# Set shape key value and insert keyframe for frame 162 (no frame jumping)
shape_key.value = 0.2734353474232214
shape_key.keyframe_insert(data_path="value", frame=162)

# Set shape key value and insert keyframe for frame 168 (no frame jumping)
shape_key.value = 1.4807647236625412
shape_key.keyframe_insert(data_path="value", frame=168)

# Set shape key value and insert keyframe for frame 174 (no frame jumping)
shape_key.value = 0.9386055313066417
shape_key.keyframe_insert(data_path="value", frame=174)

# Set shape key value and insert keyframe for frame 180 (no frame jumping)
shape_key.value = -1.0702805469998546
shape_key.keyframe_insert(data_path="value", frame=180)

# Set shape key value and insert keyframe for frame 186 (no frame jumping)
shape_key.value = -1.875451563571926
shape_key.keyframe_insert(data_path="value", frame=186)

# Set shape key value and insert keyframe for frame 192 (no frame jumping)
shape_key.value = -1.1713078255700815
shape_key.keyframe_insert(data_path="value", frame=192)

# Set shape key value and insert keyframe for frame 198 (no frame jumping)
shape_key.value = 0.35408596721147045
shape_key.keyframe_insert(data_path="value", frame=198)

# Set shape key value and insert keyframe for frame 204 (no frame jumping)
shape_key.value = 1.788718878115076
shape_key.keyframe_insert(data_path="value", frame=204)

# Set shape key value and insert keyframe for frame 210 (no frame jumping)
shape_key.value = 2.0826155314076678
shape_key.keyframe_insert(data_path="value", frame=210)

# Set shape key value and insert keyframe for frame 216 (no frame jumping)
shape_key.value = 0.4607476398659953
shape_key.keyframe_insert(data_path="value", frame=216)

# Set shape key value and insert keyframe for frame 222 (no frame jumping)
shape_key.value = -0.9327057178990394
shape_key.keyframe_insert(data_path="value", frame=222)

# Set shape key value and insert keyframe for frame 228 (no frame jumping)
shape_key.value = -0.9146556152072882
shape_key.keyframe_insert(data_path="value", frame=228)

# Set shape key value and insert keyframe for frame 234 (no frame jumping)
shape_key.value = -0.3419373894611524
shape_key.keyframe_insert(data_path="value", frame=234)


# Animate Displace.001 using SMOOTH keyframe insertion (no frame jumping)
...
# Set shape key value and insert keyframe for frame 6 (no frame jumping)
shape_key.value = -0.2310274788458011
shape_key.keyframe_insert(data_path="value", frame=6)

# Set shape key value and insert keyframe for frame 12 (no frame jumping)
shape_key.value = -1.561438234643038
shape_key.keyframe_insert(data_path="value", frame=12)
...
# Set shape key value and insert keyframe for frame 54 (no frame jumping)
shape_key.value = -1.798564328976489
shape_key.keyframe_insert(data_path="value", frame=54)
...
# Set shape key value and insert keyframe for frame 114 (no frame jumping)
shape_key.value = -1.7887250375527826
shape_key.keyframe_insert(data_path="value", frame=114)

# Set shape key value and insert keyframe for frame 120 (no frame jumping)
shape_key.value = -1.624496690220865
shape_key.keyframe_insert(data_path="value", frame=120)

# Set shape key value and insert keyframe for frame 126 (no frame jumping)
shape_key.value = -1.5426853519152894
shape_key.keyframe_insert(data_path="value", frame=126)

# Set shape key value and insert keyframe for frame 132 (no frame jumping)
shape_key.value = -1.5468339709799108
shape_key.keyframe_insert(data_path="value", frame=132)

# Set shape key value and insert keyframe for frame 138 (no frame jumping)
shape_key.value = -1.5750993510086624
shape_key.keyframe_insert(data_path="value", frame=138)

# Set shape key value and insert keyframe for frame 144 (no frame jumping)
shape_key.value = -1.5917608880572207
shape_key.keyframe_insert(data_path="value", frame=144)

# Set shape key value and insert keyframe for frame 150 (no frame jumping)
shape_key.value = -1.6084709780337947
shape_key.keyframe_insert(data_path="value", frame=150)

# Set shape key value and insert keyframe for frame 156 (no frame jumping)
shape_key.value = -1.6118102760527901
shape_key.keyframe_insert(data_path="value", frame=156)

# Set shape key value and insert keyframe for frame 162 (no frame jumping)
shape_key.value = -1.5588245489696013
shape_key.keyframe_insert(data_path="value", frame=162)

# Set shape key value and insert keyframe for frame 168 (no frame jumping)
shape_key.value = -1.3220829385461952
shape_key.keyframe_insert(data_path="value", frame=168)

# Set shape key value and insert keyframe for frame 174 (no frame jumping)
shape_key.value = -0.9232015038566219
shape_key.keyframe_insert(data_path="value", frame=174)

# Set shape key value and insert keyframe for frame 180 (no frame jumping)
shape_key.value = -0.6048215701455035
shape_key.keyframe_insert(data_path="value", frame=180)

# Set shape key value and insert keyframe for frame 186 (no frame jumping)
shape_key.value = -0.493856190704435
shape_key.keyframe_insert(data_path="value", frame=186)

# Set shape key value and insert keyframe for frame 192 (no frame jumping)
shape_key.value = -0.6189202123598281
shape_key.keyframe_insert(data_path="value", frame=192)

# Set shape key value and insert keyframe for frame 198 (no frame jumping)
shape_key.value = -0.6600272560076093
shape_key.keyframe_insert(data_path="value", frame=198)

# Set shape key value and insert keyframe for frame 204 (no frame jumping)
shape_key.value = -0.44975738558377865
shape_key.keyframe_insert(data_path="value", frame=204)

# Set shape key value and insert keyframe for frame 210 (no frame jumping)
shape_key.value = -0.10737092725106878
shape_key.keyframe_insert(data_path="value", frame=210)

# Set shape key value and insert keyframe for frame 216 (no frame jumping)
shape_key.value = 0.12211011165526808
shape_key.keyframe_insert(data_path="value", frame=216)

# Set shape key value and insert keyframe for frame 222 (no frame jumping)
shape_key.value = 0.06317290606017667
shape_key.keyframe_insert(data_path="value", frame=222)

# Set shape key value and insert keyframe for frame 228 (no frame jumping)
shape_key.value = -0.22154404305509945
shape_key.keyframe_insert(data_path="value", frame=228)

# Set shape key value and insert keyframe for frame 234 (no frame jumping)
shape_key.value = -0.4807075882325502
shape_key.keyframe_insert(data_path="value", frame=234)


# Animate Displace.002 using SMOOTH keyframe insertion (no frame jumping)
...
# Set shape key value and insert keyframe for frame 6 (no frame jumping)
shape_key.value = 0.03476669400524668
shape_key.keyframe_insert(data_path="value", frame=6)

# Set shape key value and insert keyframe for frame 12 (no frame jumping)
shape_key.value = -0.5277298966247127
shape_key.keyframe_insert(data_path="value", frame=12)

# Set shape key value and insert keyframe for frame 18 (no frame jumping)
shape_key.value = -0.3277180818102745
shape_key.keyframe_insert(data_path="value", frame=18)

# Set shape key value and insert keyframe for frame 24 (no frame jumping)
shape_key.value = -0.16712036494260563
shape_key.keyframe_insert(data_path="value", frame=24)

# Set shape key value and insert keyframe for frame 30 (no frame jumping)
shape_key.value = -0.6313483368977479
shape_key.keyframe_insert(data_path="value", frame=30)

# Set shape key value and insert keyframe for frame 36 (no frame jumping)
shape_key.value = -1.1538103593210842
shape_key.keyframe_insert(data_path="value", frame=36)

# Set shape key value and insert keyframe for frame 42 (no frame jumping)
shape_key.value = -1.0244720343166924
shape_key.keyframe_insert(data_path="value", frame=42)

# Set shape key value and insert keyframe for frame 48 (no frame jumping)
shape_key.value = -0.5966739400632177
shape_key.keyframe_insert(data_path="value", frame=48)

# Set shape key value and insert keyframe for frame 54 (no frame jumping)
shape_key.value = -0.21631575699839156
shape_key.keyframe_insert(data_path="value", frame=54)

# Set shape key value and insert keyframe for frame 60 (no frame jumping)
shape_key.value = -0.3252528032697086
shape_key.keyframe_insert(data_path="value", frame=60)

# Set shape key value and insert keyframe for frame 66 (no frame jumping)
shape_key.value = -1.100626142845126
shape_key.keyframe_insert(data_path="value", frame=66)
...
# Set shape key value and insert keyframe for frame 90 (no frame jumping)
shape_key.value = -1.2182233843798551
shape_key.keyframe_insert(data_path="value", frame=90)

# Set shape key value and insert keyframe for frame 96 (no frame jumping)
shape_key.value = -1.039063184135225
shape_key.keyframe_insert(data_path="value", frame=96)

# Set shape key value and insert keyframe for frame 102 (no frame jumping)
shape_key.value = -1.2194106278626184
shape_key.keyframe_insert(data_path="value", frame=102)
...
# Set shape key value and insert keyframe for frame 114 (no frame jumping)
shape_key.value = -1.4660248951934
shape_key.keyframe_insert(data_path="value", frame=114)

# Set shape key value and insert keyframe for frame 120 (no frame jumping)
shape_key.value = -1.1442541899902436
shape_key.keyframe_insert(data_path="value", frame=120)

# Set shape key value and insert keyframe for frame 126 (no frame jumping)
shape_key.value = -0.7943931808292122
shape_key.keyframe_insert(data_path="value", frame=126)

# Set shape key value and insert keyframe for frame 132 (no frame jumping)
shape_key.value = -0.6059828249053154
shape_key.keyframe_insert(data_path="value", frame=132)

# Set shape key value and insert keyframe for frame 138 (no frame jumping)
shape_key.value = -0.8074834051994041
shape_key.keyframe_insert(data_path="value", frame=138)

# Set shape key value and insert keyframe for frame 144 (no frame jumping)
shape_key.value = -1.2284146996655279
shape_key.keyframe_insert(data_path="value", frame=144)

# Set shape key value and insert keyframe for frame 150 (no frame jumping)
shape_key.value = -1.3381977469862854
shape_key.keyframe_insert(data_path="value", frame=150)

# Set shape key value and insert keyframe for frame 156 (no frame jumping)
shape_key.value = -1.15991688826476
shape_key.keyframe_insert(data_path="value", frame=156)

# Set shape key value and insert keyframe for frame 162 (no frame jumping)
shape_key.value = -0.8044951948384237
shape_key.keyframe_insert(data_path="value", frame=162)

# Set shape key value and insert keyframe for frame 168 (no frame jumping)
shape_key.value = -0.3614179540554836
shape_key.keyframe_insert(data_path="value", frame=168)

# Set shape key value and insert keyframe for frame 174 (no frame jumping)
shape_key.value = -0.0562093223572564
shape_key.keyframe_insert(data_path="value", frame=174)

# Set shape key value and insert keyframe for frame 180 (no frame jumping)
shape_key.value = -0.23421992333010877
shape_key.keyframe_insert(data_path="value", frame=180)

# Set shape key value and insert keyframe for frame 186 (no frame jumping)
shape_key.value = -0.46372953557526886
shape_key.keyframe_insert(data_path="value", frame=186)

# Set shape key value and insert keyframe for frame 192 (no frame jumping)
shape_key.value = -0.2873458302795542
shape_key.keyframe_insert(data_path="value", frame=192)

# Set shape key value and insert keyframe for frame 198 (no frame jumping)
shape_key.value = 0.0892854617899137
shape_key.keyframe_insert(data_path="value", frame=198)

# Set shape key value and insert keyframe for frame 204 (no frame jumping)
shape_key.value = 0.5023724580829674
shape_key.keyframe_insert(data_path="value", frame=204)

# Set shape key value and insert keyframe for frame 210 (no frame jumping)
shape_key.value = 0.6979253870810362
shape_key.keyframe_insert(data_path="value", frame=210)

# Set shape key value and insert keyframe for frame 216 (no frame jumping)
shape_key.value = 0.33577319498536506
shape_key.keyframe_insert(data_path="value", frame=216)

# Set shape key value and insert keyframe for frame 222 (no frame jumping)
shape_key.value = -0.08533401492662773
shape_key.keyframe_insert(data_path="value", frame=222)

# Set shape key value and insert keyframe for frame 228 (no frame jumping)
shape_key.value = -0.15324135972075337
shape_key.keyframe_insert(data_path="value", frame=228)

# Set shape key value and insert keyframe for frame 234 (no frame jumping)
shape_key.value = -0.10600466571711475
shape_key.keyframe_insert(data_path="value", frame=234)


# Animate Displace.003 using SMOOTH keyframe insertion (no frame jumping)
shape_key = cube.data.shape_keys.key_blocks["Displace.003"]

# Set shape key value and insert keyframe for frame 0 (no frame jumping)
shape_key.value = 0.8218117481704736
shape_key.keyframe_insert(data_path="value", frame=0)

# Set shape key value and insert keyframe for frame 6 (no frame jumping)
shape_key.value = -0.07094104480598329
shape_key.keyframe_insert(data_path="value", frame=6)

# Set shape key value and insert keyframe for frame 12 (no frame jumping)
shape_key.value = -0.12871007129920375
shape_key.keyframe_insert(data_path="value", frame=12)

# Set shape key value and insert keyframe for frame 18 (no frame jumping)
shape_key.value = 0.42204042559401
shape_key.keyframe_insert(data_path="value", frame=18)

# Set shape key value and insert keyframe for frame 24 (no frame jumping)
shape_key.value = 0.28824078230777217
shape_key.keyframe_insert(data_path="value", frame=24)

# Set shape key value and insert keyframe for frame 30 (no frame jumping)
shape_key.value = -1.0831289702487696
shape_key.keyframe_insert(data_path="value", frame=30)

# Set shape key value and insert keyframe for frame 36 (no frame jumping)
shape_key.value = -1.5712286911030588
shape_key.keyframe_insert(data_path="value", frame=36)

# Set shape key value and insert keyframe for frame 42 (no frame jumping)
shape_key.value = -0.7225293981871761
shape_key.keyframe_insert(data_path="value", frame=42)

# Set shape key value and insert keyframe for frame 48 (no frame jumping)
shape_key.value = 0.6707163623661139
shape_key.keyframe_insert(data_path="value", frame=48)

# Set shape key value and insert keyframe for frame 54 (no frame jumping)
shape_key.value = 1.598850437652943
shape_key.keyframe_insert(data_path="value", frame=54)

# Set shape key value and insert keyframe for frame 60 (no frame jumping)
shape_key.value = 1.3812174131967183
shape_key.keyframe_insert(data_path="value", frame=60)

# Set shape key value and insert keyframe for frame 66 (no frame jumping)
shape_key.value = -0.3260996557342377
shape_key.keyframe_insert(data_path="value", frame=66)

# Set shape key value and insert keyframe for frame 72 (no frame jumping)
shape_key.value = -1.4068548583202651
shape_key.keyframe_insert(data_path="value", frame=72)

# Set shape key value and insert keyframe for frame 78 (no frame jumping)
shape_key.value = -1.0935397564074474
shape_key.keyframe_insert(data_path="value", frame=78)

# Set shape key value and insert keyframe for frame 84 (no frame jumping)
shape_key.value = -0.5266398800533727
shape_key.keyframe_insert(data_path="value", frame=84)

# Set shape key value and insert keyframe for frame 90 (no frame jumping)
shape_key.value = -0.32299988340972696
shape_key.keyframe_insert(data_path="value", frame=90)

# Set shape key value and insert keyframe for frame 96 (no frame jumping)
shape_key.value = -0.7285365890467004
shape_key.keyframe_insert(data_path="value", frame=96)

# Set shape key value and insert keyframe for frame 102 (no frame jumping)
shape_key.value = -1.638070319625609
shape_key.keyframe_insert(data_path="value", frame=102)
...
# Set shape key value and insert keyframe for frame 114 (no frame jumping)
shape_key.value = -1.373429961775671
shape_key.keyframe_insert(data_path="value", frame=114)

# Set shape key value and insert keyframe for frame 120 (no frame jumping)
shape_key.value = -0.5483676923895973
shape_key.keyframe_insert(data_path="value", frame=120)

# Set shape key value and insert keyframe for frame 126 (no frame jumping)
shape_key.value = -0.1709573924503156
shape_key.keyframe_insert(data_path="value", frame=126)

# Set shape key value and insert keyframe for frame 132 (no frame jumping)
shape_key.value = -0.3100834528063636
shape_key.keyframe_insert(data_path="value", frame=132)

# Set shape key value and insert keyframe for frame 138 (no frame jumping)
shape_key.value = -1.0328697299472989
shape_key.keyframe_insert(data_path="value", frame=138)

# Set shape key value and insert keyframe for frame 144 (no frame jumping)
shape_key.value = -1.6141384753371633
shape_key.keyframe_insert(data_path="value", frame=144)

# Set shape key value and insert keyframe for frame 150 (no frame jumping)
shape_key.value = -1.249536883698978
shape_key.keyframe_insert(data_path="value", frame=150)

# Set shape key value and insert keyframe for frame 156 (no frame jumping)
shape_key.value = -0.16418422524334475
shape_key.keyframe_insert(data_path="value", frame=156)

# Set shape key value and insert keyframe for frame 162 (no frame jumping)
shape_key.value = 0.7815432572120684
shape_key.keyframe_insert(data_path="value", frame=162)

# Set shape key value and insert keyframe for frame 168 (no frame jumping)
shape_key.value = 0.9180257149836935
shape_key.keyframe_insert(data_path="value", frame=168)

# Set shape key value and insert keyframe for frame 174 (no frame jumping)
shape_key.value = 0.026831003801572884
shape_key.keyframe_insert(data_path="value", frame=174)

# Set shape key value and insert keyframe for frame 180 (no frame jumping)
shape_key.value = -0.7407555916601072
shape_key.keyframe_insert(data_path="value", frame=180)

# Set shape key value and insert keyframe for frame 186 (no frame jumping)
shape_key.value = -0.5167152823312391
shape_key.keyframe_insert(data_path="value", frame=186)

# Set shape key value and insert keyframe for frame 192 (no frame jumping)
shape_key.value = 0.37989150062392363
shape_key.keyframe_insert(data_path="value", frame=192)

# Set shape key value and insert keyframe for frame 198 (no frame jumping)
shape_key.value = 1.3317744981566741
shape_key.keyframe_insert(data_path="value", frame=198)

# Set shape key value and insert keyframe for frame 204 (no frame jumping)
shape_key.value = 1.7887514778647378
shape_key.keyframe_insert(data_path="value", frame=204)

# Set shape key value and insert keyframe for frame 210 (no frame jumping)
shape_key.value = 1.2501019360294892
shape_key.keyframe_insert(data_path="value", frame=210)

# Set shape key value and insert keyframe for frame 216 (no frame jumping)
shape_key.value = 0.10095013571247802
shape_key.keyframe_insert(data_path="value", frame=216)

# Set shape key value and insert keyframe for frame 222 (no frame jumping)
shape_key.value = -0.18286331073416573
shape_key.keyframe_insert(data_path="value", frame=222)

# Set shape key value and insert keyframe for frame 228 (no frame jumping)
shape_key.value = 0.09928580169420154
shape_key.keyframe_insert(data_path="value", frame=228)

# Set shape key value and insert keyframe for frame 234 (no frame jumping)
shape_key.value = 0.3139111088345372
shape_key.keyframe_insert(data_path="value", frame=234)
...
for fcurve in action.fcurves:
    ...
    # Apply ULTRA-SMOOTH interpolation for dramatic but smooth shape changes
    for keyframe in fcurve.keyframe_points:
        keyframe.interpolation = 'BEZIER'
        
        if interpolation_type == 'organic':
            # Organic interpolation with longer handles for smoother transitions
            keyframe.handle_left_type = 'FREE'
            keyframe.handle_right_type = 'FREE'
            keyframe.handle_left[0] = -0.6  # Longer handles for smoother curves
            keyframe.handle_right[0] = 0.6
            keyframe.handle_left[1] = keyframe.co[1] * 0.25  # More influence
            keyframe.handle_right[1] = keyframe.co[1] * 0.25
        elif interpolation_type == 'smooth':
            # Smooth interpolation with auto handles
            keyframe.handle_left_type = 'AUTO'
            keyframe.handle_right_type = 'AUTO'
        elif interpolation_type == 'fluid':
            # Fluid interpolation for seamless morphing
            keyframe.handle_left_type = 'FREE'
            keyframe.handle_right_type = 'FREE'
            keyframe.handle_left[0] = -0.5
            keyframe.handle_right[0] = 0.5
            keyframe.handle_left[1] = keyframe.co[1] * 0.2
            keyframe.handle_right[1] = keyframe.co[1] * 0.2
        elif interpolation_type == 'rhythmic':
            # Rhythmic interpolation for musical responsiveness
            keyframe.handle_left_type = 'FREE'
            keyframe.handle_right_type = 'FREE'
            keyframe.handle_left[0] = -0.4
            keyframe.handle_right[0] = 0.4
            keyframe.handle_left[1] = keyframe.co[1] * 0.3
            keyframe.handle_right[1] = keyframe.co[1] * 0.3
        else:
...
# NO ROTATION ANIMATION - Keep cube stationary to prevent jumping
# Only shape morphing through shape keys, no rotation or scaling
cube.rotation_euler = (0, 0, 0)
cube.location = (0, 0, 0)
cube.scale = (1, 1, 1)  # Fixed scale - no size changes
